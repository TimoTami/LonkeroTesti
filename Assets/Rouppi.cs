...
public class Rouppi : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (rope&&hiiri== true && (Input.GetMouseButtonDown(0)|| Input.GetKeyDown("4")))
        {
            hiiri = false;
            poikki = false;
            var ff = joints;
        ...
        else if (!rope && hiiri==false && kameraScript.crossi==kameraScript.osuuCrossi && (Input.GetMouseButtonDown(0)||Input.GetKeyDown("4")))
        {
            kameraScript.GetTarget1(ref targetPoint, ref parentGameObject);

            //targetPoint= kameraScript.place;
            //parentGameObject = kameraScript.placeGameObject;
        ...
        if (rope && Input.GetKey("a"))
        {
            LengthenRope();
            //LengthenRope1();
        }
        else if (rope && Input.GetKey("q"))
        {
    ...
    void LateUpdate()
    {
        ...
        {
            Debug.Log("start :" + line.startWidth + " end: " + line.endWidth);
            line.enabled = false;
            if (!lineriPoikkinaiselle)
            {
    ...
    void BuildRope()
    {
        segments = (int)(Vector3.Distance(transform.position, targetPointGameobject.transform.position) * resolution);
        line.positionCount = segments;
        segmentPos = new Vector3[segments];
        joints = new GameObject[segments];
        segmentPos[0] = transform.position;
        segmentPos[segments - 1] = targetPointGameobject.transform.position;
        var seperation = ((targetPointGameobject.transform.position - transform.position) / segments);
        original = segments;
    ...
    void LengthenRope()
    {
        if (joints.Length < 120&&poikki==false)
        {
            ...
            dis = targetPointGameobject.transform.position - segmentPos[segmentPos.Length - 3];
